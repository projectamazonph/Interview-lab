...
function fallbackFeedback(question: string, responseText = ''): CoachResult {
  return {
    score: 5,
    ...
    strongerSampleAnswer:
      'Please review the question and try to include specific metrics, processes, and Amazon terminology in your answer.',
...
function errorFeedback(): CoachResult {
  return {
    score: 5,
    ...
    strongerSampleAnswer:
      'A strong answer would include specific metrics (ACoS, ROAS, CTR, CVR), mention relevant processes (keyword harvesting, search term review, bid optimization), and demonstrate understanding of Amazon workflows.',
...
export const coachConfig: AIHandlerConfig<CoachBody, CoachResult> = {
  ...
  validate: (body) => {
    ...
    if (!shape.ok) {
      return { ok: false, status: 400, error: 'Question and answer are required' };
    }
    return { ok: true, value: body as CoachBody };
  },
  ...
  onParseFailure: (body) => {
    // On parse failure the original route returned a graceful fallback (200), not a 500.
    void body;
    return { ok: true, value: fallbackFeedback(body.question) };
  },
  normalize: (result, body) => {
    // Ensure followUpQuestion is always present (mirrors original route logic).
    if (!result.followUpQuestion) {
      const score = typeof result.score === 'number' ? result.score : 5;
      result.followUpQuestion =
        score < 7
          ? `Your answer could use more detail. Can you try again, specifically focusing on the process steps and any relevant metrics for: "${body.question}"?`
          : `Good answer! As a follow-up, can you explain how you would handle a more complex scenario related to this topic?`;
    }
    return result;
  },
