"use client";

import React, { useState, useEffect } from "react";
import { useAuth } from "@/lib/auth-context";
import { DashboardData, ActiveView } from "@/lib/types";
import { GlassCard } from "@/components/ui/glass-card";
import { GlassButton } from "@/components/ui/glass-button";
import { GlassBadge } from "@/components/ui/glass-badge";
import { Progress } from "@/components/ui/progress";
import { Badge } from "@/components/ui/badge";
import { motion } from "framer-motion";
import { fadeUpVariants, staggerContainer } from "@/lib/animations";
import {
  Question,
  ChatsCircle,
  Target,
  TrendUp,
  ArrowUpRight,
  Lightning,
  Clock,
} from "@phosphor-icons/react";

interface DashboardViewProps {
  onViewChange: (view: ActiveView) => void;
}

export function DashboardView({ onViewChange }: DashboardViewProps) {
  const { user } = useAuth();
  const [data, setData] = useState<DashboardData | null>(null);
  const [loading, setLoading] = useState(true);
  const [questionCount, setQuestionCount] = useState(0);

  useEffect(() => {
    if (!user) return;
    fetch("/api/dashboard", { headers: { "x-user-id": user.id } })
      .then((res) => res.json())
      .then((d) => setData(d))
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [user]);

  useEffect(() => {
    fetch("/api/questions/count")
      .then((res) => res.json())
      .then((data) => setQuestionCount(data.total || 0))
      .catch(() => {});
  }, []);

  if (loading) {
    return (
      <div className="space-y-6">
        <div className="space-y-4">
          <div className="h-8 bg-glass-border/30 rounded-full w-1/4 shimmer" />
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-24 bg-glass-border/20 rounded-2xl shimmer" />
            ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-32 bg-glass-border/20 rounded-2xl shimmer" />
            ))}
          </div>
        </div>
      </div>
    );
  }

  const stats = data?.stats || {
    totalSessions: 0,
    completedSessions: 0,
    totalAttempts: 0,
    avgScore: 0,
    latestResumeScore: null,
  };
  const profile = data?.profile;
  const isNewUser = stats.totalAttempts === 0 && stats.completedSessions === 0;

  return (
    <motion.div
      variants={staggerContainer}
      initial="hidden"
      animate="visible"
      className="space-y-6"
    >
      {/* Welcome */}
      <motion.div variants={fadeUpVariants}>
        <GlassBadge variant="accent" className="mb-3">
          <Lightning className="w-3 h-3 mr-1" weight="light" /> Your Dashboard
        </GlassBadge>
        <h2 className="font-heading text-2xl sm:text-3xl font-bold text-text-primary">
          Welcome back{user?.name ? `, ${user.name.split(" ")[0]}` : ""}!
        </h2>
        <p className="text-text-muted mt-1 text-sm">
          {profile?.targetRole
            ? `Preparing for ${profile.targetRole} roles`
            : "Your Amazon VA interview prep hub"}
        </p>
      </motion.div>

      {/* Empty State */}
      {isNewUser && (
        <motion.div variants={fadeUpVariants}>
          <GlassCard variant="elevated" className="p-8 text-center">
            <div className="w-16 h-16 rounded-2xl bg-accent-violet/15 flex items-center justify-center mx-auto mb-4">
              <Target className="w-8 h-8 text-accent-indigo" weight="light" />
            </div>
            <h3 className="font-heading text-xl font-bold text-text-primary mb-2">Ready to start your prep?</h3>
            <p className="text-text-muted text-sm mb-6 max-w-md mx-auto">
              Begin with a mock interview or browse questions to see what agencies ask.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <GlassButton onClick={() => onViewChange("interview")}>
                Start Mock Interview <ArrowUpRight className="w-4 h-4" weight="light" />
              </GlassButton>
              <GlassButton variant="secondary" onClick={() => onViewChange("questions")}>
                Browse Questions
              </GlassButton>
            </div>
          </GlassCard>
        </motion.div>
      )}

      {/* Stats Grid */}
      <motion.div variants={fadeUpVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: "Questions Practiced",
            value: stats.totalAttempts,
            sub: `/${questionCount}`,
            icon: Question,
            color: "text-accent-indigo",
            bg: "bg-accent-violet/15",
          },
          {
            label: "Mock Interviews",
            value: stats.completedSessions,
            sub: "",
            icon: ChatsCircle,
            color: "text-accent-emerald",
            bg: "bg-accent-emerald/15",
          },
          {
            label: "Avg. Score",
            value: stats.avgScore > 0 ? stats.avgScore.toFixed(1) : "—",
            sub: "/10",
            icon: TrendUp,
            color: "text-accent-amber",
            bg: "bg-accent-amber/15",
          },
          {
            label: "Resume Score",
            value: stats.latestResumeScore ? stats.latestResumeScore.toFixed(0) : "—",
            sub: "/100",
            icon: Target,
            color: "text-accent-indigo",
            bg: "bg-accent-violet/15",
          },
        ].map((stat, i) => (
          <GlassCard key={i} variant="interactive" className="p-5">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl ${stat.bg} flex items-center justify-center shrink-0`}>
                <stat.icon className={`w-5 h-5 ${stat.color}`} weight="light" />
              </div>
              <div className="min-w-0">
                <p className="text-2xl font-bold text-text-primary font-heading">
                  {stat.value}
                  <span className="text-sm font-normal text-text-muted">{stat.sub}</span>
                </p>
                <p className="text-xs text-text-muted truncate">{stat.label}</p>
              </div>
            </div>
          </GlassCard>
        ))}
      </motion.div>

      {/* Quick Actions */}
      <motion.div variants={fadeUpVariants}>
        <h3 className="font-heading font-semibold text-text-primary text-lg mb-4">Quick Actions</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { title: "Mock Interview", desc: "AI-coached interview session", icon: ChatsCircle, action: () => onViewChange("interview") },
            { title: "Browse Questions", desc: `${questionCount || 222}+ questions available`, icon: Question, action: () => onViewChange("questions") },
            { title: "Practice Test", desc: "Timed skill assessment", icon: Target, action: () => onViewChange("assessments") },
          ].map((item, i) => (
            <GlassCard key={i} variant="interactive" className="p-6 cursor-pointer" onClick={item.action}>
              <div className="w-10 h-10 rounded-xl bg-glass-border/30 flex items-center justify-center mb-3">
                <item.icon className="w-5 h-5 text-accent-indigo" weight="light" />
              </div>
              <h4 className="font-heading font-semibold text-text-primary text-sm mb-1">{item.title}</h4>
              <p className="text-text-muted text-xs">{item.desc}</p>
            </GlassCard>
          ))}
        </div>
      </motion.div>

      {/* Learning Path Progress */}
      {profile?.targetRole && (
        <motion.div variants={fadeUpVariants}>
          <GlassCard className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-heading font-semibold text-text-primary">Your Learning Path</h3>
                <p className="text-text-muted text-sm">{profile.targetRole}</p>
              </div>
              <GlassBadge variant="accent">
                {Math.min(Math.round(stats.avgScore * 10), 100)}% Ready
              </GlassBadge>
            </div>
            <Progress
              value={Math.min(Math.round(stats.avgScore * 10), 100)}
              className="h-2 bg-glass-border/30"
            />
            {profile.weakAreas && (
              <div className="flex flex-wrap gap-2 mt-4">
                <span className="text-text-muted text-xs">Focus areas:</span>
                {(() => {
                  let areas: string[] = [];
                  try {
                    const raw = profile.weakAreas;
                    areas = Array.isArray(raw) ? raw : typeof raw === "string" ? JSON.parse(raw) : [];
                  } catch { areas = []; }
                  return areas.map((area: string, i: number) => (
                    <GlassBadge key={i} variant="warning" className="text-[10px]">{area}</GlassBadge>
                  ));
                })()}
              </div>
            )}
          </GlassCard>
        </motion.div>
      )}

      {/* Recent Sessions */}
      {(data?.recentSessions?.length || 0) > 0 && (
        <motion.div variants={fadeUpVariants}>
          <GlassCard className="p-6">
            <h3 className="font-heading font-semibold text-text-primary mb-4">Recent Sessions</h3>
            <div className="space-y-3">
              {data?.recentSessions?.slice(0, 5).map((session) => (
                <div
                  key={session.id}
                  className="flex items-center justify-between p-4 rounded-xl bg-glass-border/20 border border-glass-border/30 gap-2"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Clock className="w-4 h-4 text-text-muted shrink-0" weight="light" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-text-primary capitalize truncate">
                        {session.mode.replace(/_/g, " ")}
                      </p>
                      <p className="text-xs text-text-muted">
                        {new Date(session.startedAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {session.overallScore !== null && session.overallScore !== undefined && (
                      <GlassBadge
                        variant={session.overallScore >= 7 ? "success" : session.overallScore >= 5 ? "warning" : "danger"}
                      >
                        {session.overallScore.toFixed(1)}/10
                      </GlassBadge>
                    )}
                    <GlassBadge variant={session.completedAt ? "accent" : "muted"}>
                      {session.completedAt ? "Done" : "Active"}
                    </GlassBadge>
                  </div>
                </div>
              ))}
            </div>
          </GlassCard>
        </motion.div>
      )}
    </motion.div>
  );
}
