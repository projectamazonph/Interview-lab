 ...
 *
 * Auth: Uses session cookies (interviewlab_session) instead of x-user-id header.
 * Login POST returns Set-Cookie, subsequent requests replay it.
 */

const BASE_URL = process.env.TEST_BASE_URL || 'http://localhost:3000';
const COOKIE_NAME = 'interviewlab_session';
...
// Cookie jar: maps user email -> cookie string for that session
const cookieJar = {};

async function api(method, path, body, userEmail) {
  const headers = { 'Content-Type': 'application/json' };
  if (userEmail && cookieJar[userEmail]) {
    headers['Cookie'] = cookieJar[userEmail];
  }
  ...
  // Capture Set-Cookie from login responses
  if (userEmail && res.headers.get('set-cookie')) {
    const setCookie = res.headers.get('set-cookie');
    // Extract interviewlab_session cookie value
    const match = setCookie.match(new RegExp(`${COOKIE_NAME}=([^;]+)`));
    if (match) {
      cookieJar[userEmail] = `${COOKIE_NAME}=${match[1]}`;
    }
  }
...
const ADMIN_EMAIL = 'admin@interviewlab.com';
...
async function testAuth() {
  ...
  assert(!!cookieJar[DEMO_EMAIL], 'Login sets session cookie');
...
async function testProfile() {
  ...
  let r = await api('GET', '/api/profile', undefined, DEMO_EMAIL);
  assert(r.status === 401, 'GET profile requires auth', `got ${r.status}`);

  r = await api('GET', '/api/profile', undefined, DEMO_EMAIL);
...
async function testDashboard() {
  ...
  let r = await api('GET', '/api/dashboard', undefined, DEMO_EMAIL);
  assert(r.status === 401, 'Dashboard requires auth', `got ${r.status}`);

  r = await api('GET', '/api/dashboard', undefined, DEMO_EMAIL);
...
async function testInterviews() {
  ...
  // List
  r = await api('GET', '/api/interview', undefined, DEMO_EMAIL);
  ...
  if (sessionId) {
    ...
    r = await api('GET', `/api/interview/${sessionId}`, undefined, DEMO_EMAIL);
    ...
    // Complete
    r = await api('POST', `/api/interview/${sessionId}/complete`, {}, DEMO_EMAIL);
    ...
    r = await api('GET', `/api/interview/${sessionId}`, undefined, ADMIN_EMAIL);
...
async function testResumes() {
  ...
  r = await api('GET', '/api/resume', undefined, DEMO_EMAIL);
  ...
  if (resumeId) {
    ...
    r = await api('GET', `/api/resume/${resumeId}`, undefined, DEMO_EMAIL);
    ...
    // Ownership check
    await getAdminUserId();
    r = await api('GET', `/api/resume/${resumeId}`, undefined, ADMIN_EMAIL);
...
async function testCoverLetters() {
  ...
  r = await api('GET', '/api/cover-letter', undefined, DEMO_EMAIL);
  ...
  if (clId) {
    ...
    r = await api('GET', `/api/cover-letter/${clId}`, undefined, DEMO_EMAIL);
...
async function testGuides() {
  ...
  // Progress
  await getDemoUserId();
  r = await api('GET', '/api/guides/progress', undefined, DEMO_EMAIL);
...
async function testAdmin() {
  ...
  // Admin access
  let r = await api('GET', '/api/admin/questions', undefined, ADMIN_EMAIL);
  assert(r.status === 200, 'Admin can list questions', `got ${r.status}`);

  r = await api('GET', '/api/admin/questions', undefined, DEMO_EMAIL);
...
async function testExport() {
  ...
  r = await api('POST', '/api/export', {}, DEMO_EMAIL);
...
async function main() {
  ...
  try {
    ...
    console.error('❌ Server not available. Start with: bun run start');
    process.exit(1);
  }
  ...
  try { await testInterviews(); } catch (e) { console.error('Interviews tests error:', e); }
